 ...
 * 15_ListMaintain.js      … 入力アプリ「リスト追加」からの作業／部品／作業者の追記・上書き。
 *
 * 【中項目】
 * 入力アプリの選択肢。選ぶと作業内容を順番どおり明細にする。中項目名自体は明細に出さない。
 * シート「車検_入力」「中項目候補」「中項目_参照」は使わない。
 *
 * 【作業リストの列】
 * A 大項目 / B 中項目 / C 作業内容 / D 技術料 / E 作業コード / F 部品_中項目 / G 単価 / H 数量 / （他） / 最後が順番。
 * 技術料は作業内容の行に付ける（リスト追加と同じ）。中項目の先頭行（作業内容が空）には付けない。
 * 作業内容が無いパックは、リスト追加で1行目の作業内容に中項目名を入れる。2行目以降の技術料のみは保存できない。
 * 昔の「中項目に技術料」は読込・リスト更新時に、パック内の先頭の空行だけ作業内容＝中項目名にする。
 * 作業コードは作業者リストと同じ数値コード。氏名と作業内容が一致すると空欄だけ自動入力。
 * 選択肢は「コード：作業者」。選ぶとセルには作業コードだけ入る（テンプレートリストと同じ）。
 * E と部品_中項目はリスト選択＋手入力可（既存値は消さない）。
 *
 * 【順番列】
 * 「順番」または「表示順」。リスト更新時は 大項目で固め → 中項目で固め → 中項目内は順番の昇順（空は後）。
 * 大項目同士は塊の最小順番。中項目同士はシート上の出順（手で塊を動かして決める。更新しても入れ替えない）。
 * 中項目の中のレーン（更新忘れの仮順番用）:
 *   0 先頭行（1 行だけ。作業内容が空。無ければ順番 1 の空行）
 *   1 作業内容（順番昇順）
 *   2 追加の部品セット（作業内容が空で部品あり。順番昇順）
 * 空の順番は更新ボタンのときだけシートへ書く。手入力は上書きしない。
 * 更新は図形ボタン（refreshAllMasterLists）またはメニュー。
...
const CONFIG = {
  ...
  serviceInfo: {
    /** A＝全て。B〜E＝種別1〜4。F＝受付メール。G＝受付担当。H・I＝値引％初期値。J＝登録地名。 */
    sheetName: '整備情報',
    defaultTechPct: 3,
    defaultPartPct: 10
  },
   ...
   * ヘッダー初期値は整備部門・整備種別。同じ名前のうち順番が一番小さい行の値。空欄は上書きしない。
   * 順番はテンプレート名ごとに 10,20,30…。更新時に振り直す。
